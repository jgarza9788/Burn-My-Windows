...
void main() {
  ...
  //progress is 0.0 and 1
  float time     = progress * uDuration;
  // not sure if i need this variable

  vec2 uv = iTexCoord ;

  float dist = length(uv - uStartPos);

  float ring00 = sin(dist - progress)
  ring00 = 1- ring00; 

  //remap amplitude value
  //float mapped_value = mix(min_out, max_out, (value - min_in) / (max_in - min_in));
  float amp00 = mix(100, 10, (uAmplitude - 0) / (100 - 0));
  float pow00 = pow(ring00,amp00);
  vec2 disUV = uv - pow00;
  vec4 oColor = getInputColor(iTexCoord, disUV);


  float CS00 = mix(100, 5, (uColorSize - 0) / (100 - 0));
  float pow01 = pow(ring00, CS00 )
  vec4 color00 = pow01 * uColor;
  oColor = oColor + color00;


  float mask = (uv - uStartPos) - progress;
