...
export default class Effect {
  ...
  constructor() {
    this.shaderFactory = new ShaderFactory(Effect.getNick(), (shader) => {
      ...
      shader._uMinSize = shader.get_uniform_location("uMinSize");
      shader._uStrength = shader.get_uniform_location("uStrength");

      // Write all uniform values at the start of each animation.
      shader.connect("begin-animation", (shader, settings) => {

        shader.set_uniform_float(shader._uMinSize, 1, [
          settings.get_double("simple-fade-min-size"),
        ]);

        shader.set_uniform_float(shader._uStrength, 1, [
          settings.get_double("simple-fade-strength"),
        ]);
  ...
  static bindPreferences(dialog) {
    ...
    dialog.bindAdjustment('simple-fade-min-size');
    dialog.bindAdjustment('simple-fade-strength');
  }
